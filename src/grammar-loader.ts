...
function evalCond(cond: any, rule: any): boolean {
  for (const key of Object.keys(cond)) {
    if ('$and' === key) {
      for (const sub of cond.$and) {
        if (!evalCond(sub, rule)) return false
      }
      continue
    }
    if ('$or' === key) {
      ...
      for (const sub of cond.$or) {
        if (evalCond(sub, rule)) { any = true; break }
      }
      if (!any) return false
      continue
    }
    ...
    if ('$not' === key) {
      if (evalCond(cond.$not, rule)) return false
      continue
    }
    ...
    if (spec != null && 'object' === typeof spec) {
      ...
      if ('$eq' in spec && numVal !== spec.$eq) return false
      if ('$ne' in spec && numVal === spec.$ne) return false
      if ('$gt' in spec && !(numVal > spec.$gt)) return false
      if ('$gte' in spec && !(numVal >= spec.$gte)) return false
      if ('$lt' in spec && !(numVal < spec.$lt)) return false
      if ('$lte' in spec && !(numVal <= spec.$lte)) return false
    } else {
...
function resolveToken(ref: string, jsonic: any): any {
  ...
  if (tin != null && 'number' === typeof tin) return tin

  return ref
}

function resolveTokenMatch(match: any, jsonic: any): any {
...
function resolveTokenSeq(s: any[], jsonic: any): any[] {
  return s.map((pos: any) => resolveTokenMatch(pos, jsonic))
}
...
function buildCond(condSpec: any, funcs: any): any {
  if ('string' === typeof condSpec) {
    return funcs[condSpec]
  }
  // MongoDB-style query object
  return (rule: any) => evalCond(condSpec, rule)
}
  ...
  return (rule: any, ctx: any, next: any) => {
    ...
    if (bindMode != null) {
      ...
      if (bindGuard != null && !evalCond(bindGuard, rule)) {
        ...
        if (bindNullify && undefined === val) val = null
...
function buildAltAction(altDecl: any, funcs: any, jsonic: any): any {
  const hasKey = true === altDecl.key
  const hasPush = 'push' in altDecl
  const hasFuncRef = altDecl.a != null

  if (!hasKey && !hasPush && !hasFuncRef) return undefined
  ...
  return (rule: any, ctx: any, alt: any) => {
    if (hasKey) {
      const t = rule.o0
...
function convertAlt(altDecl: any, funcs: any, jsonic: any, cfg: any): any {
  // Config gate
  ...
  if (altDecl.p != null) alt.p = altDecl.p
  if (altDecl.r != null) alt.r = altDecl.r
  ...
  if (altDecl.n != null) alt.n = altDecl.n
  if (altDecl.u != null) alt.u = { ...altDecl.u }
  if (altDecl.k != null) alt.k = { ...altDecl.k }
  if (altDecl.g != null) alt.g = altDecl.g
...
function applyGrammar(jsonic: any, spec: any, funcs?: any): void {
  funcs = funcs || {}
  ...
  for (const ruleName of Object.keys(spec.rules)) {
    ...
    jsonic.rule(ruleName, (rs: any, parser: any) => {
      ...
      if (ruleDecl.ao) rs.ao(funcs[ruleDecl.ao])
      if (ruleDecl.ac) rs.ac(funcs[ruleDecl.ac])

      // Open alternates
      if (ruleDecl.open) {
        for (const group of ruleDecl.open) {
          const alts = group.alts
            .map((a: any) => convertAlt(a, funcs, jsonic, parserCfg))
            .filter((a: any) => false !== a)

          const flags: any = group.mods ? { ...group.mods } : undefined
          rs.open(alts, flags)
        }
      }
