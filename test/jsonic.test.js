...
describe('jsonic', function () {
  ...
  it('basic-json', () => {
    ...
    expect(Jsonic('{"a":true,"b":false,"c":null}')).toEqual({a:true,b:false,c:null})
  ...
  it('basic-object-tree', () => {
    ...
    expect(Jsonic('{x:10,a:{b:{c:{d:2}}}}')).toEqual({x:10,a:{b:{c:{d:2}}}})

    expect(Jsonic('{a:{b:2},y:20}')).toEqual({a:{b:2},y:20})
    expect(Jsonic('{a:{b:{c:2}},y:20}')).toEqual({a:{b:{c:2}},y:20})
    expect(Jsonic('{a:{b:{c:{d:2}}},y:20}')).toEqual({a:{b:{c:{d:2}}},y:20})

    expect(Jsonic('{x:10,a:{b:2},y:20}')).toEqual({x:10,a:{b:2},y:20})
    expect(Jsonic('{x:10,a:{b:{c:2}},y:20}')).toEqual({x:10,a:{b:{c:2}},y:20})
    expect(Jsonic('{x:10,a:{b:{c:{d:2}}},y:20}')).toEqual({x:10,a:{b:{c:{d:2}}},y:20})

    expect(Jsonic('{a:{b:2,c:3}}')).toEqual({a:{b:2,c:3}})
    expect(Jsonic('{a:{b:2,c:3,d:4}}')).toEqual({a:{b:2,c:3,d:4}})
    expect(Jsonic('{a:{b:{e:2},c:3,d:4}}')).toEqual({a:{b:{e:2},c:3,d:4}})
    expect(Jsonic('{a:{b:2,c:{e:3},d:4}}')).toEqual({a:{b:2,c:{e:3},d:4}})
    expect(Jsonic('{a:{b:2,c:3,d:{e:4}}}')).toEqual({a:{b:2,c:3,d:{e:4}}})
    
    expect(Jsonic('{a:{b:{c:2,d:3}}}')).toEqual({a:{b:{c:2,d:3}}})
    expect(Jsonic('{a:{b:{c:2,d:3,e:4}}}')).toEqual({a:{b:{c:2,d:3,e:4}}})
    expect(Jsonic('{a:{b:{c:{f:2},d:3,e:4}}}')).toEqual({a:{b:{c:{f:2},d:3,e:4}}})
    expect(Jsonic('{a:{b:{c:2,d:{f:3},e:4}}}')).toEqual({a:{b:{c:2,d:{f:3},e:4}}})
    expect(Jsonic('{a:{b:{c:2,d:3,e:{f:4}}}}')).toEqual({a:{b:{c:2,d:3,e:{f:4}}}})
  ...
  it('basic-array-tree', () => {
    ...
    expect(Jsonic('[[2,9],0,[4,9],1,[3,9]]')).toEqual([[2,9],0,[4,9],1,[3,9]])
  ...
  it('process-object-tree', () => {
    ...
    expect(j('a:1,b:{c:2},d:3,e:{f:4}')).toEqual({a:1, d:3, b:{c:2}, e:{f:4}})
    expect(j('a:1,b:{c:2},d:3,e:{f:4},g:5'))
      .toEqual({a:1, d:3, b:{c:2}, e:{f:4}, g:5})
    ...
    expect(j('a:1,b:{c:2},d:{e:{f:3}}'))
      .toEqual({a:1, b:{c:2}, d:{e:{f:3}}})
    expect(j('a:1,b:{c:2},d:{e:{f:3}},g:4'))
      .toEqual({a:1, b:{c:2}, d:{e:{f:3}}, g:4})
    expect(j('a:1,b:{c:2},d:{e:{f:3}},h:{i:5},g:4'))
      .toEqual({a:1, b:{c:2}, d:{e:{f:3}}, g:4, h:{i:5}})
  ...
  it('process-mixed-nodes', () => {
    ...
    expect(j('[{a:[{b:1}]},{a:[{b:1}]}]')).toEqual([{a:[{b:1}]},{a:[{b:1}]}])
    expect(j('[{a:{b:[1]}},{a:{b:[1]}}]')).toEqual([{a:{b:[1]}},{a:{b:[1]}}])
    expect(j('[{a:{b:[{c:1}]}},{a:{b:[{c:1}]}}]'))
      .toEqual([{a:{b:[{c:1}]}},{a:{b:[{c:1}]}}])
    expect(j('[{a:{b:{c:[1]}}},{a:{b:{c:[1]}}}]'))
      .toEqual([{a:{b:{c:[1]}}},{a:{b:{c:[1]}}}])
  })
  ...
  it('process-comment', () => {
    expect(j('a:q\nb:w #X\nc:r \n\nd:t\n\n#')).toEqual({a:'q',b:'w',c:'r',d:'t'})

    let jm = j.make({comment: { lex: false }})
    expect(jm('a:q\nb:w#X\nc:r \n\nd:t')).toEqual({a: 'q', b: 'w#X', c: 'r', d: 't'})
  })
